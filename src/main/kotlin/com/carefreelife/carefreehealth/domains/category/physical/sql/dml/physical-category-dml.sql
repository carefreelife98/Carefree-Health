...
ALTER TABLE physical_category AUTO_INCREMENT = 1;



# 특정 카테고리의 직속 하위 카테고리 찾기
SELECT c.category_name
FROM physical_category_closure AS cc
         JOIN physical_category AS c
             ON cc.sub_cat_id = c.physical_category_id
...
FROM physical_category_closure AS cc
         JOIN physical_category AS c
              ON cc.parent_cat_id = c.physical_category_id
...
FROM physical_category_closure AS cc
         JOIN physical_category AS ancestor_c
             ON cc.parent_cat_id = ancestor_c.physical_category_id
         JOIN physical_category AS descendant_c
             ON cc.sub_cat_id = descendant_c.physical_category_id
...
### test
INSERT INTO physical_category (category_name)
VALUES ('벤치 프레스');

INSERT INTO physical_category_closure (parent_cat_id, sub_cat_id)
